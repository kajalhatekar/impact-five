...
type DrawSummary = {
  total_draws_entered: number | string;
  upcoming_draw_id: string | null;
  upcoming_draw_month: string | null;
  upcoming_draw_status: string | null;
};
...
function formatDrawMonth(date: string | null) {
  if (!date) {
    return null;
  }

  const parsedDate = new Date(`${date.slice(0, 10)}T00:00:00Z`);
...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.rpc("get_my_draw_summary"),
  ]);
  ...
  const drawSummaryRows =
    (drawSummaryResult.data as DrawSummary[] | null) ?? [];

  const drawSummary = drawSummaryRows[0] ?? null;

  const totalDrawsEntered = Number(
    drawSummary?.total_draws_entered ?? 0,
  );

  const upcomingDrawMonth = formatDrawMonth(
    drawSummary?.upcoming_draw_month ?? null,
  );

  const upcomingDrawStatus =
    drawSummary?.upcoming_draw_status ?? null;
  ...
  const loadError =
    ...
    drawSummaryResult.error?.message ??
    null;
      ...
      <section className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <article className="flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {isSubscriptionActive ? (
            <>
              ...
              <Link
                href="/subscribe"
                className="mt-auto inline-flex pt-5 text-sm font-semibold text-emerald-700 transition hover:text-emerald-900"
              >
                Manage membership →
              </Link>
            </>
            ...
            <>
              ...
              <Link
                href="/subscribe"
                className="mt-auto inline-flex rounded-full bg-emerald-700 px-4 py-2 text-xs font-semibold text-white transition hover:bg-emerald-800"
              >
                Choose a plan
              </Link>
            </>
        ...
        <article className="flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {winnerCount > 0 ? (
            ...
            <span className="mt-auto inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
              No winnings yet
            </span>
          )}
        </article>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <div className="rounded-2xl bg-emerald-50 p-4">
              ...
              <p className="mt-2 font-bold text-emerald-950">
                {upcomingDrawMonth ?? "Not scheduled"}
              </p>

              <p className="mt-1 text-xs text-emerald-700">
                {upcomingDrawStatus === "simulated"
                  ? "Results awaiting publication"
                  : upcomingDrawStatus
                    ? "Entry preparation in progress"
                    : "The administrator has not configured the next draw."}
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-slate-100 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                {totalDrawsEntered === 1
                  ? "1 recorded draw entry"
                  : `${totalDrawsEntered} recorded draw entries`}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                {subscriptionPrice}/{billingInterval}
                {subscriptionPlanCode ? ` · ${subscriptionPlanCode} plan` : ""}
              </p>
